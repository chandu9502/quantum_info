...
The **Quantum Fourier Transform** converts the periodic structure into frequency information.
...
### 3. Use QFT to extract the period

\[
\boxed{\text{QFT} \rightarrow \text{find }r}
\]

### 4. Use GCD to obtain factors

\[
\boxed{\gcd(a^{r/2}\pm1,N)\rightarrow\text{factors}}
\]

### In one sentence

> **Shor's algorithm uses quantum Fourier analysis to find the period of \(a^x\bmod N\), then uses that period to obtain the factors of \(N\).**
